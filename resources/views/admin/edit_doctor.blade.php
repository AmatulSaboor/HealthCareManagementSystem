...
<div class="container" class ="index">
    ...
    <form action="{{url('doctor'. '/' .$doctor->id )}}" method="POST" class="form-container">
        ...
        <input id="experience" type="text" name ="experience" value="{{old('experience', $doctor->experience)}}"/>
        @error('experience')
        <div class="alert alert-danger">{{$message}}</div>
        @enderror
        <label for="dob">Date of Birth</label>
        <input id="dob" type="date" name ="dob" value="{{old('dob', $doctor->dob)}}" min="{{date('Y-m-d', strtotime("-80 years", strtotime(date('Y-m-d'))))}}"  max="{{date('Y-m-d', strtotime("-20 years", strtotime(date('Y-m-d'))))}}" />
        @error('dob')
        <div class="alert alert-danger">{{$message}}</div>
        @enderror
        <label>Gender</label>
        ...
        <input id="start_time" type="time" name ="start_time" value="{{old('start_time', $doctor->start_time)}}"/>
        @error('start_time')
        <div class="alert alert-danger">{{$message}}</div>
        @enderror
        <label for="end_time">End Time</label>
        <input id="end_time" type="time" name ="end_time" value="{{old('end_time', $doctor->end_time)}}"/>
        @error('end_time')
        <div class="alert alert-danger">{{$message}}</div>
        @enderror
        <label for="charges">Charges</label>
        <input id="charges" type="text" name ="charges" value="{{old('charges', $doctor->charges)}}"/>
        @error('charges')
        <div class="alert alert-danger">{{$message}}</div>
        @enderror
